...
type HistoryTab = "bias" | "snapshots" | "resolved" | "add_result";
    ...
    try {
      ...
      const response = await fetch("/api/weather/history/resolved", {
        ...
        body: JSON.stringify({
          ...
          resolvedBucket:
            resultForm.resolvedBucket.trim() || bucketFromTempF(Number(resultForm.resolvedHighF)),
          ...
          {snapshots.length ? (
            <div className="space-y-4">
              {snapshots.map((snapshot) => (
                <SnapshotCard key={snapshot.id ?? `${snapshot.eventTicker}-${snapshot.createdAt}`} snapshot={snapshot} />
              ))}
            </div>
          ) : (
...
function SnapshotCard({ snapshot }: { snapshot: WeatherForecastSnapshotDocument }) {
  return (
